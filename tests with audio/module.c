...
#define PLUGIN_NAME "modulemixer"

#define KMS_module_MIXER_LOCK(mixer) \
  (g_rec_mutex_lock (&( (KmsmoduleMixer *) (mixer))->priv->mutex))

#define KMS_module_MIXER_UNLOCK(mixer) \
  (g_rec_mutex_unlock (&( (KmsmoduleMixer *) (mixer))->priv->mutex))

GST_DEBUG_CATEGORY_STATIC (kms_module_mixer_debug_category);
#define GST_CAT_DEFAULT kms_module_mixer_debug_category

#define KMS_module_MIXER_GET_PRIVATE(obj) (\
  G_TYPE_INSTANCE_GET_PRIVATE (               \
    (obj),                                    \
    KMS_TYPE_module_MIXER,                 \
    KmsmoduleMixerPrivate                  \
  )                                           \
)
...
G_DEFINE_TYPE_WITH_CODE (KmsmoduleMixer, kms_module_mixer,
    KMS_TYPE_BASE_HUB,
    GST_DEBUG_CATEGORY_INIT (kms_module_mixer_debug_category, PLUGIN_NAME,
        0, "debug category for modulemixer element"));

typedef struct _KmsmoduleMixerData
{
  KmsRefStruct parent;
  gint id;
  KmsmoduleMixer *mixer;
...
#define KMS_module_MIXER_REF(data) \
  kms_ref_struct_ref (KMS_REF_STRUCT_CAST (data))
#define KMS_module_MIXER_UNREF(data) \
  kms_ref_struct_unref (KMS_REF_STRUCT_CAST (data))

static void
kms_destroy_module_mixer_data (KmsmoduleMixerData * data)
{
  g_slice_free (KmsmoduleMixerData, data);
}

static KmsmoduleMixerData *
kms_create_module_mixer_data ()
{
  KmsmoduleMixerData *data;

  data = g_slice_new0 (KmsmoduleMixerData);
  kms_ref_struct_init (KMS_REF_STRUCT_CAST (data),
      (GDestroyNotify) kms_destroy_module_mixer_data);
...
{
  KmsmoduleMixerData *port_data_a = (KmsmoduleMixerData *) a;
  KmsmoduleMixerData *port_data_b = (KmsmoduleMixerData *) b;

  return port_data_a->id - port_data_b->id;
}

static void
kms_module_mixer_recalculate_sizes (gpointer data)
{
  KmsmoduleMixer *self = KMS_module_MIXER (data);
  GstCaps *filtercaps;
  ...
  for (l = values; l != NULL; l = l->next) {
    KmsmoduleMixerData *port_data = l->data;
...
static gboolean
remove_elements_from_pipeline (KmsmoduleMixerData * port_data)
{
  KmsmoduleMixer *self = port_data->mixer;

  KMS_module_MIXER_LOCK (self);
...
{
  KmsmoduleMixerData *port_data = (KmsmoduleMixerData *) data;
  KmsmoduleMixer *self = port_data->mixer;
  GstEvent *event;
  ...
  kms_loop_idle_add_full (self->priv->loop, G_PRIORITY_DEFAULT,
      ...
      KMS_module_MIXER_REF (port_data),
...
static void
kms_module_mixer_port_data_destroy (gpointer data)
{
  KmsmoduleMixerData *port_data = (KmsmoduleMixerData *) data;
  KmsmoduleMixer *self = port_data->mixer;
  GstPad *audiosink;
  ...
  if (port_data->input) {
    ...
    if (!GST_OBJECT_FLAG_IS_SET (pad, GST_PAD_FLAG_EOS)) {
      ...
      if (port_data->input && self->priv->n_elems > 0) {
        ...
        self->priv->n_elems--;
        kms_module_mixer_recalculate_sizes (self);
      }
      KMS_module_MIXER_UNLOCK (self);
    ...
    KmsmoduleMixerData * data)
{
  ...
  mixer = KMS_module_MIXER (data->mixer);
  GST_DEBUG ("stream start detected %d", data->id);
  KMS_module_MIXER_LOCK (mixer);
  ...
  data->probe_id = gst_pad_add_probe (data->video_mixer_pad,
      ...
      KMS_module_MIXER_REF (data), (GDestroyNotify) kms_ref_struct_unref);
  ...
  mixer->priv->n_elems++;
  kms_module_mixer_recalculate_sizes (mixer);
...
static void
kms_module_mixer_unhandle_port (KmsBaseHub * mixer, gint id)
{
  KmsmoduleMixer *self = KMS_module_MIXER (mixer);
  ...
  KMS_BASE_HUB_CLASS (G_OBJECT_CLASS
      (kms_module_mixer_parent_class))->unhandle_port (mixer, id);
}

static KmsmoduleMixerData *
kms_module_mixer_port_data_create (KmsmoduleMixer * mixer, gint id)
{
  KmsmoduleMixerData *data;
  GstPad *tee_src;
  GstCaps *filtercaps;
  ...
  data = kms_create_module_mixer_data ();
  data->mixer = mixer;
  data->id = id;
  data->input = FALSE;
  data->removing = FALSE;
...
static gint
kms_module_mixer_handle_port (KmsBaseHub * mixer,
    GstElement * mixer_end_point)
{
  KmsmoduleMixer *self = KMS_module_MIXER (mixer);
  KmsmoduleMixerData *port_data;
  gint port_id;

  GST_DEBUG ("Handle new HubPort");
  port_id = KMS_BASE_HUB_CLASS (G_OBJECT_CLASS
      (kms_module_mixer_parent_class))->handle_port (mixer, mixer_end_point);
  ...
  if (self->priv->videomixer == NULL) {
    ...
    g_object_set (G_OBJECT (self->priv->videomixer), "background",
        1, "start-time-selection", 1 /*first */ ,
  ...
  port_data = kms_module_mixer_port_data_create (self, port_id);
...
static void
kms_module_mixer_dispose (GObject * object)
{
  KmsmoduleMixer *self = KMS_module_MIXER (object);
  ...
  G_OBJECT_CLASS (kms_module_mixer_parent_class)->dispose (object);
}

static void
kms_module_mixer_finalize (GObject * object)
{
  KmsmoduleMixer *self = KMS_module_MIXER (object);
  ...
  G_OBJECT_CLASS (kms_module_mixer_parent_class)->finalize (object);
}

static void
kms_module_mixer_class_init (KmsmoduleMixerClass * klass)
{
  ...
  gst_element_class_set_static_metadata (GST_ELEMENT_CLASS (klass),
      "moduleMixer", "Generic", "Mixer element that composes n input flows"
      " in one output flow", "David Fernandez <d.fernandezlop@gmail.com>");

  gobject_class->dispose = GST_DEBUG_FUNCPTR (kms_module_mixer_dispose);
  gobject_class->finalize = GST_DEBUG_FUNCPTR (kms_module_mixer_finalize);

  base_hub_class->handle_port =
      GST_DEBUG_FUNCPTR (kms_module_mixer_handle_port);
  base_hub_class->unhandle_port =
      GST_DEBUG_FUNCPTR (kms_module_mixer_unhandle_port);
  ...
  g_type_class_add_private (klass, sizeof (KmsmoduleMixerPrivate));
}

static void
kms_module_mixer_init (KmsmoduleMixer * self)
{
  self->priv = KMS_module_MIXER_GET_PRIVATE (self);

  g_rec_mutex_init (&self->priv->mutex);

  self->priv->ports = g_hash_table_new_full (g_int_hash, g_int_equal,
      release_gint, kms_module_mixer_port_data_destroy);
...
gboolean
kms_module_mixer_plugin_init (GstPlugin * plugin)
{
